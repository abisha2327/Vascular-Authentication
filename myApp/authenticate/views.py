    ...
    if request.method == 'POST':
        ...
        if Palm.objects.filter(email=mail).exists():
            ...
            if isinstance(im, InMemoryUploadedFile):
                en = Palm(accnum=accnum, acctype=acctype, username=username, phnumber=ph, email=mail, picture=im)
                en.save()
...
def home(request):
    ...
    data = {}
    try:
        data['accnum'] = Palm.objects.filter(username=username).first().accnum
